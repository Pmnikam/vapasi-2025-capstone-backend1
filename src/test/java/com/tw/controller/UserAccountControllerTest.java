...
@DisplayName("✅ Unit test UserAccountController without @MockBean")
class UserAccountControllerTest {
    ...
    void shouldReturn400BadRequestForInvalidInputOnSignup() throws Exception {
        RegisterUserDto invalidDto = new RegisterUserDto("", "invalidEmail", ""); // Invalid fields
    ...
    void shouldReturn409ConflictWhenEmailAlreadyExistsOnSignup() throws Exception {
        ...
        mockMvc.perform(post("/users/signup")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isConflict()) // or .is(HttpStatus.CONFLICT.value())
    ...
    void shouldReturn400BadRequestWhenPasswordIsTooShortOnSignup() throws Exception {
        ...
        mockMvc.perform(post("/users/signup")
                        ...
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.password").value("Password is required and must be at least 6 characters long"));
    }
    ...
    void shouldReturn401UnauthorizedForInvalidCredentialsOnLogin() throws Exception {
        ...
        Mockito.when(userService.authenticate(any()))
                .thenThrow(new InvalidUserCredentialsException("Invalid email or password"));
